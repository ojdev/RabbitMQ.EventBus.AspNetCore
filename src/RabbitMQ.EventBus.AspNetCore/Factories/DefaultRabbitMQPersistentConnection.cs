...
internal sealed class DefaultRabbitMQPersistentConnection : IRabbitMQPersistentConnection
{
    ...
    private static DefaultRabbitMQPersistentConnection Instance;
    private static object singleton_Lock = new object();
    public static DefaultRabbitMQPersistentConnection CreateInstance(RabbitMQEventBusConnectionConfiguration configuration, Func<string> connectionAction, ILogger<DefaultRabbitMQPersistentConnection> logger)
    {
        lock (singleton_Lock) // 保证任意时刻只有一个线程才能进入判断
        {
            if (Instance == null)
            {
                Instance = new DefaultRabbitMQPersistentConnection(configuration, connectionAction, logger);
            }
        }
        return Instance;
    }
    private DefaultRabbitMQPersistentConnection(RabbitMQEventBusConnectionConfiguration configuration, Func<string> connectionAction, ILogger<DefaultRabbitMQPersistentConnection> logger)
    {
